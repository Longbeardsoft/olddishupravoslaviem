...
//новый пункт меню в консоли
/*function render_admin_page() {
    echo '<h1>Настройка плагина Test Plugin</h1>';
}*/
function test_plugin_add_top_menu() {
    add_menu_page( 'Лишний пункт еще пригодиться', 'Лишний пункт еще пригодиться', 'manage_options', 
    'zhitiya-svyatyx/zhitiya-svyatyx.php',
	'',	'',
	//plugins_url( 'zhitiya-svyatyx/images/icon.png' ),
	1
	);
}
...
//перенос скриптов из head в футер
//no
...
function theme_update_styles() {
	...
	wp_register_style("css", 'http://fonts.googleapis.com/css?family=Arvo|PT+Sans&subset=latin', array(), false, "all");
...
function theme_get_metadata_icons($icons = '', $class = '') {
    ...
    for ($i = 0; $i < count($icons); $i++) {
        ...
        switch ($icon) {
            case 'date':
                $result[] = '<span class="inputpostdateicon">' . sprintf(__('<span class="%1$s">Published</span> %2$s', THEME_NS),
                                'date',
		...
		$result[] ='';
                ...
                $result[] = '<span class="inputpostauthoricon">' . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
                                'author',
            ...
            case 'category':
                ...
                if (theme_strlen($categories) == 0)
                    break;
                $result[] = '<span class="inputpostcategoryicon">' . sprintf(__('<span class="%1$s">Posted in</span> %2$s', THEME_NS), 'categories', get_the_category_list(', ')) . '</span>';
                break;
            case 'tag':
                $tags_list = get_the_tag_list('', ', ');
                if (!$tags_list)
                    break;
                $result[] = '<span class="inputposttagicon">' . sprintf(__('<span class="%1$s">Tagged</span> %2$s', THEME_NS), 'tags', $tags_list) . '</span>';
                break;
            case 'comments':
                ...
                $result[] = '<span class="inputpostcommentsicon">' . theme_ob_get_clean() . '</span>';
                break;
            case 'edit':
                ...
                $result[] = '<span class="inputpostediticon">' . theme_ob_get_clean() . '</span>';
                break;
        }
    }
    ...
    if (theme_is_empty_html($result))
        return;
    return "<div class=\"inputpost{$class}icons inputmetadata-icons\">{$result}</div>";
}
...
function theme_get_content($args = array()) {
    global $wp_query;
    $post_id = get_queried_object_id();
    $more_tag = theme_get_array_value($args, 'more_tag', __('Продолжить чтение <span class=\'meta-nav\'>&rarr;</span>', THEME_NS));
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        ...
        'focus_script' => '<script type="text/javascript">jQuery(\'div.inputcontent input[name="s"]\').focus();</script>'
            )
    );
    extract($args);
    theme_post_wrapper(
...
function theme_stylize_pagination($pagination) {
    if ($pagination) {
        $pagination = '<div class="inputpager">' . str_replace(array('current', 'dots'), array('current active', 'dots more'), $pagination) . '</div>';        
    }
    return $pagination;
}

function theme_comment_reply_link_filter($link) {
    return str_replace('class=\'', 'class=\'inputbutton ', $link);
}
...
function theme_comment($comment, $args, $depth) {
    ...
    switch ($comment->comment_type) :
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
            <div class="inputcomment <?php echo $comment->comment_type ?> clearfix" id="comment-<?php comment_ID() ?>">
    <div class="inputcomment-avatar"><?php echo str_replace (array('height="150"', 'width="150"', "height='150'", "width='150'"), array('height="100%"', 'width="100%"', "height='100%'", "width='100%'"), theme_get_avatar(array('id' => $comment, 'size' => 150))); ?></div>
    <div class="inputcomment-inner">
        <div class="inputcomment-header comment-meta commentmetadata"><?php printf(__('%s on ', THEME_NS), get_comment_author_link($comment->comment_ID)); ?>
...
<?php edit_comment_link(__('(Edit)', THEME_NS)); ?></div>
        <div class="inputcomment-content comment-body"><?php if ($comment->comment_approved == '0') : ?>
    <em><?php _e('Your comment is awaiting moderation.', THEME_NS); ?></em>
    <br />
<?php endif; ?>
<?php comment_text(); ?></div>
        <div class="inputcomment-footer reply"><?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))); ?></div>
    </div>
</div>
            <?php
            break;
        ...
        <li class="post pingback">
            <div class="inputcomment <?php echo $comment->comment_type ?> clearfix">
                <div class="inputcomment-content comment-body"><?php _e('Pingback:', THEME_NS); ?> <?php comment_author_link(); ?><?php edit_comment_link(__('(Edit)', THEME_NS), ' '); ?></div>
            </div>
            <?php
            break;
    endswitch;
...
//отключение автоформатирования конец
