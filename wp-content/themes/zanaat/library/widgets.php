...
class LoginWidget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ($user_ID):
			...
			?>
			<ul>
			<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/"><?php _e('Профиль', THEME_NS); ?></a></li>
			<?php if ($user_level >= 1): ?>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/post-new.php"><?php _e('Новая запись', THEME_NS); ?></a></li>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-admin/edit-comments.php"><?php _e('Модерация', THEME_NS); ?></a></li>
			<?php endif; ?>
			<li><a href="<?php echo wp_logout_url() ?>&amp;redirect_to=<?php echo urlencode(theme_get_current_url()); ?>"><?php _e("Выйти", THEME_NS); ?></a></li>
			</ul>
		<?php
		else:
			_e('Вход', THEME_NS);
			...
			<form action="<?php bloginfo('wpurl') ?>/wp-login.php" method="post" name="login" id="form-login">
				<fieldset class="input" style="border: 0 none;">
					<p id="form-login-username">
						<label for="log"><?php _e('Имя пользователя', THEME_NS) ?></label>
						<br>
						<input type="text" name="log" id="log" value="<?php echo esc_attr(stripslashes($user_login), 1) ?>" size="20" />
					</p>
					<p id="form-login-password">
						<label for="pwd"><?php _e("Пароль", THEME_NS); ?></label>
						<br>
						<input type="password" name="pwd" id="pwd" size="20" /><br />
					</p>
					<p id="form-login-remember">
						<label for="rememberme"><?php _e('Запомнить?', THEME_NS); ?></label>
						<input name="rememberme" id="rememberme" type="checkbox" checked="checked" value="forever" />
					</p>
					<input class="inputbutton" type="submit" name="submit" value="<?php echo esc_attr(__('Вход', THEME_NS)); ?>" />
				</fieldset>
				<input type="hidden" name="redirect_to" value="<?php echo theme_get_current_url(); ?>"/>
			</form>
			<ul>
				<?php if (get_option('users_can_register')) { ?>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-register.php"><?php _e("Регистрация", THEME_NS); ?></a></li>
				<?php } ?>
				<li><a href="<?php bloginfo('wpurl') ?>/wp-login.php?action=lostpassword"><?php _e("Забыли пароль?", THEME_NS); ?></a></li>
			</ul>
		<?php endif;
		echo $after_widget;
	}
