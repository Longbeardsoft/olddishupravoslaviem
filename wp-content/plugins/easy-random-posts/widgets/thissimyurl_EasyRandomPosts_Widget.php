...
class thissimyurl_EasyRandomPosts_Widget extends WP_Widget {
	...
	/**
	  ...
	  *
	  */
     function thissimyurl_EasyRandomPosts_Widget() {
		 
		$widget_ops = array( 
						'classname' => 'widget_thisismyurl_random_posts', 
						'description' => __( 'A WordPress widget to add random posts to any WordPress theme.', THISISMYURL_ERP_NAMESPACE ) 
					);
					
		$control_ops = array( 'width' => 300, 'height' => 300, 'id'=> 'widget_thisismyurl_random_posts' );
		
		$this->WP_Widget( 	'thissimyurl_EasyRandomPosts_Widget', 
							__( 'Easy Random Posts', THISISMYURL_ERP_NAMESPACE ), 
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args( (array) $instance, $thissimyurl_EasyRandomPosts->random_posts_defaults() );
	...
	function widget( $args, $instance ) {
		...
		$random_posts = $thissimyurl_EasyRandomPosts->easy_random_posts( $instance );
		...
		if ( ! empty ( $random_posts ) ) {
			...
			if ( $instance['show_credit'] == 1 )	
				$title = sprintf( '<a 	href="http://thisismyurl.com/downloads/easy-random-posts/" 
										title="' . esc_attr( __( 'Easy Random Posts', THISISMYURL_ERP_NAME ) ). '">%s</a>',
								  $title
						  ); 
